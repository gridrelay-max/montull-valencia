...
function votesRef(member: string): DatabaseReference | null {
  if (!db) return null;
  return ref(db, `votes/${member}`);
}

function allVotesRef(): DatabaseReference | null {
  if (!db) return null;
  return ref(db, "votes");
}
...
function VotingApp({ member, onSwitchMember }: { member: string; onSwitchMember: () => void }) {
  ...
  useEffect(() => {
    setLoading(true);
    const myRef = votesRef(member);
    const allRef = allVotesRef();

    if (!myRef || !allRef) {
      // Firebase not connected — still allow local voting
      setLoading(false);
      return;
    }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0A0A0A" }}>
      ...
      {firebaseError && (
        <div style={{
          background: "#3A1A0A", borderBottom: "1px solid #C75B3940",
          padding: "12px 20px", textAlign: "center",
        }}>
          <p style={{ fontSize: 13, color: "#E8A87C", margin: 0 }}>
            ⚠️ Base de datos no conectada — los votos solo se guardan en esta sesión.
          </p>
          <p style={{ fontSize: 11, color: "#8B6B50", margin: "4px 0 0" }}>
            Revisa las variables de entorno en Vercel. Error: {firebaseError}
          </p>
        </div>
      )}
      {/* Header */}
